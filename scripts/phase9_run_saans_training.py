...
def main() -> None:
    ...
    # Make this a real run configuration rather than a tiny smoke-test.
    cfg.exp_name = "kaggle_saans"
    cfg.batch_size = 32
    cfg.num_workers = 2
    cfg.test_epochs = 1
    ...
    for step, batch in enumerate(runtime.dataloaders["train"]):
        ...
        train_losses.append(result.weighted_loss)
        tracker_history.append(list(tracker.values))

        if step % 50 == 0:
            print(
                f"SAANS step {step}: weighted_loss={result.weighted_loss:.4f}, "
    ...
    with open(out_dir / "kaggle_saans_training_summary.json", "w", encoding="utf-8") as handle:
        json.dump(
            {
                "summary": summary,
    ...
    print("Saved SAANS training summary to:", out_dir / "kaggle_saans_training_summary.json")
    print(summary)

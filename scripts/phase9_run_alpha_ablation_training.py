...
def main() -> None:
    ...
    cfg.exp_name = "kaggle_saans_alpha05"
    cfg.batch_size = 32
    cfg.num_workers = 2
    cfg.test_epochs = 1
    ...
    for step, batch in enumerate(runtime.dataloaders["train"]):
        ...
        if step % 50 == 0:
            print(
                f"Alpha-0.5 step {step}: weighted_loss={result.weighted_loss:.4f}, "
    ...
    with open(out_dir / "kaggle_saans_alpha05_summary.json", "w", encoding="utf-8") as handle:
        json.dump(
            {
                "summary": summary,
    ...
    print("Saved alpha ablation summary to:", out_dir / "kaggle_saans_alpha05_summary.json")
    print(summary)
